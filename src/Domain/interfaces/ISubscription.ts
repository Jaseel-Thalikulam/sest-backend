...
export interface ISubscription {
  createSubscription(SubscriptionDetails: SubscriptionDTO): Promise<Subscription>;
  addSubscriptiontoUser(subscriptionId: ObjectId, StudentId: string): Promise<{success:boolean}>;
  getSubscriptionData(SubscriptionDetail: getSubscriptionDetailDTO): Promise<Subscription>;
  isAlreadySubscribed(PaymentDetails: PaymentDTO): Promise<boolean>;
  DeleteExpiredSubscription(subscriptionId: ObjectId): Promise<{ success: boolean; plan: string }>;
}
